...
SELECT p.title, b.year, c.name AS country  
FROM program AS p
JOIN broadcast AS b ON p.id = b.program_id
JOIN country AS c ON b.country_id = c.id
WHERE b.year >= 2010 AND c.name = "Singapore"
ORDER BY b.year ASC, p.title ASC;
...
SELECT b.year, COUNT(b.program_id) AS total_broadcast, c.name AS country
FROM broadcast AS b
JOIN country AS c ON b.country_id = c.id
GROUP BY b.year, c.name;

-- Show the total number of program broadcast by category
SELECT c.name AS category, COUNT(l.program_id) AS total_program
FROM category AS c
