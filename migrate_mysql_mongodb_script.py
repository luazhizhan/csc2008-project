...
mysql_database = "csc2008"
mysql_schema = "csc2008"
mysql_user = "root"
...
def migrate_table(db, col_name):
    ...
    # map myresult date data to mongodb date format
    for row in myresult:
        for key in row:
            if isinstance(row[key], datetime.date):
                row[key] = datetime.datetime.combine(
                    row[key], datetime.datetime.min.time())
    ...
    if len(myresult) > 0:
        x = collection.insert_many(myresult)
...
table_list_cursor.execute(
    "SELECT table_name FROM information_schema.tables WHERE table_schema = %s ORDER BY table_name LIMIT 15;", (mysql_schema,))
...
for table in tables:
    try:
        ...
        print(
            f"{bcolors.OKGREEN}Processing table: {table[0]} completed. {inserted_count} documents inserted.{bcolors.ENDC}")
        success_count += 1
